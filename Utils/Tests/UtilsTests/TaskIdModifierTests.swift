...
final class TaskIdModifierTests: XCTestCase {
    class Content: Equatable {
        static func == (lhs: TaskIdModifierTests.Content, rhs: TaskIdModifierTests.Content) -> Bool {
            lhs.title == rhs.title
        }
    ...
    func testTaskIdInspection() async throws {
        ...
        let val: Content = .init(title: "hey")
        let sut = EmptyView()
            .padding()
            .task(id: val) {
                expectedValue = val
        ...
        try await sut.inspect()
            .emptyView()
            .callTask(id: val)
        XCTAssertEqual(expectedValue?.title, "expected")
    }
